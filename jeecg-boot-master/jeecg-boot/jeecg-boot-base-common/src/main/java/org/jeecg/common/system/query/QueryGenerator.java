...
import org.jeecg.common.relation.service.impl.RelationServiceImpl;
import org.jeecg.common.relationsp.service.IRelationspService;
import org.jeecg.common.relationsp.service.impl.RelationspServiceImpl;
...
//import org.jeecg.modules.demo.relation.service.*;
...
public class QueryGenerator {
	...
	private static void addEasyQuery(QueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value) {
		...
		case LE:
			...
			break;
		case EQ:
			//String managerN=converRuleValue(dataRule.getRuleValue());
			IRelationService relationService=new RelationServiceImpl();

			//System.out.println(QueryGenerator.ac.getBean(RelationServiceImpl.class));
			Set<String> stlist=relationService.getAllsonID(value.toString());

			System.out.println("当前用户所有的子节点：包含中间节点及叶节点"+"\r\n");
			...
			IRelationspService iRelationspService=new RelationspServiceImpl();
			Set<String> rlist=iRelationspService.getDirectsonID(value.toString());

			System.out.println("当前用户的特殊关系的节点：只有一层进行单独配置的节点"+"\r\n");
			Iterator iterator1=rlist.iterator();
			while (iterator1.hasNext())
			{
				System.out.println((String) iterator1.next()+"\r\n");
			}

			stlist.addAll(rlist);//2个集合进行合并
			System.out.println("2个节点合并之后，该角色管控的人员总数量为"+stlist.size()+"\r\n");

			String[] stringlist=stlist.toArray(new String[stlist.size()]);

			System.out.println("2个节点合并之后，该角色管控的所有人员为"+"\r\n");
			for(int i=0;i<stringlist.length;i++)
				System.out.println(stringlist[i].toString()+"\r\n");
			//String[] stringlist={"sjk","tianchuang"};
			queryWrapper.in(name, stringlist);
			...
			queryWrapper.in(name,invalue);*/
			/*for(String st :stringlist) {
				QueryWrapper thisqueryWrapper = queryWrapper.eq(name, st);
				//thisqueryWrapper.or(name,st);
			}*/
			//处理第一个
			/*Object objectWrapper= queryWrapper.eq(name, stringlist[0]);
			Class c=objectWrapper.getClass();
			...
			for(int i=1;i<stringlist.length;i++)
			{
				...
				try {
					Method orMethod=c.getMethod("or");
					Object otemp= orMethod.invoke(c);
					Method eqMthod=c.getMethod("eq");
					Object
				}
				catch (Exception ex)
				{}
			...
			for(String st:stringlist)
			{
				Object thisqueryWrapper = queryWrapper.eq(name, st);

				thisqueryWrapper=queryWrapper.eq(name, st).or().eq(name,st);
			}*/



			//queryWrapper.eq(name, "sjk").or().eq(name,"tianchuang");
	 ...
	 * @param value        查询条件值
	 */
	private static void addEasyQueryOrBySteven(QueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value) {
		if (value == null || rule == null) {
			return;
		}
		name = oConvertUtils.camelToUnderline(name);
	...
	private static void addRuleToQueryWrapper(SysPermissionDataRule dataRule,String name, Class propertyType, QueryWrapper<?> queryWrapper) {
		...
		if(rule.equals(QueryRuleEnum.IN) && ! propertyType.equals(String.class)) {
			...
			if (propertyType.equals(String.class)) {
				...
				String managerN=converRuleValue(dataRule.getRuleValue());
			//	String[] stringlist={"sjk","tianchuang"};
				/*for(String managername:stringlist)
				addEasyQuery(queryWrapper, name, rule, managername);*/

				addEasyQuery(queryWrapper, name, rule, managerN);
