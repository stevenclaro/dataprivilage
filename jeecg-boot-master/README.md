...
addEasyQuery
在这个代码中增加relation的表，因为访问MybatisPlus存在问题。
问题的主要是静态方法暂时无法访问Spring管理的Bean对象。（实际上是可能有的，但是没有找到）
在relation中，使用Jeecg框架，自动生成了分层代码。然后在这个分层中，采用了jdbc的方式
对Entity进行改造，把@data等去掉
在接口中增加了一个方法，得到某个角色所管辖的人员。递归获取

增加一个relationspecial，使用Jeecg框架，自动生成了分层代码。在这个分层中，采用了jdbc方法。得到某个角色直接管辖另外的角色，主要是用于循环依赖的时候，把它拆除之后，就可以执行。
然后将2个集合进行合并
...
配置了角色的菜单，权限
2、后端
在静态方法中增加了筛选人员的代码，并增加relation，relationspecial的处理类，用jdbc来直接取
核心：增加了一个递归算法，从某一个人管理的人员中选择他所管控的人员。
3、数据库
增加了dailyreport，relation，relationspecial的库
